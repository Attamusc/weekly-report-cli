...
// describeProjectClientAdapter adapts the projects.Client to the input.ProjectClient interface
// This avoids circular dependencies between packages
type describeProjectClientAdapter struct {
	token  string
	logger *slog.Logger
}

// FetchProjectItems implements input.ProjectClient interface
func (a *describeProjectClientAdapter) FetchProjectItems(ctx context.Context, configInterface interface{}) ([]input.IssueRef, error) {
...
func runDescribe(cmd *cobra.Command, args []string) error {
	...
	logger.Info("Rendering output...", "rows", len(rows), "format", describeFormat)
	var output string
	if describeFormat == "detailed" {
...
// setupDescribeLogger creates a logger configured for progress output
func setupDescribeLogger(cfg *config.Config) *slog.Logger {
	if cfg.Quiet {
